...
const createSlot = async(req,res)=>{
    try {
       ...
       let newSlot = await Slot.create({
        ...
        return res.status(StatusCodes.OK).json({msg:"new slots avaialable",newSlot})
    } catch (err) {
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({msg:err.message})
    }
}
...
const updateSlot = async(req,res)=>{
    try {
        ...
        return res.status(StatusCodes.OK).json({msg:`slots updated successfully`})
    } catch (err) {
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({msg:err.message})
    }
}
...
const deleteSlot = async(req,res)=>{
    try {
        ...
        let extSlot = await Slot.findById({_id:id})
        if(!extSlot)
        return res.status(StatusCodes.NOT_FOUND).json({msg:"requested slot not found"})
        await Slot.findByIdAndDelete({_id:id})
        return res.status(StatusCodes.OK).json({msg:`slot deleteed succesfully`})
    } catch (err) {
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({msg:err.message})
    }
}
